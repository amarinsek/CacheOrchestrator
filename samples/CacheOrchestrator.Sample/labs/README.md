...
One app process with full **ops surface**: Admin API (live counters), Prometheus (`/metrics`), Admin Console (dashboard / fan-out / hints). Caching itself is still InMemory — observability does not change how OC/FC store data.
