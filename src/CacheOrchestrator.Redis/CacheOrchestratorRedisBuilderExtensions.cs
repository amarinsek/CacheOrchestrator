...
public static class CacheOrchestratorRedisBuilderExtensions
{
    ...
    /// Redis settings are read from <c>{section}:Redis</c> and optional per-provider overrides —
    /// not from core <c>CacheOrchestratorOptions</c>.
    /// </remarks>
        ...
        string configSection = "Cache")
    {
        ...
        return builder.AddBackend(new RedisCacheBackendRegistrar());
    }
}
