...
internal sealed class DomainRuntimeOverrideStore : IDomainRuntimeOverrideStore
{
    ...
    internal static DomainRuntimeOverride Merge(DomainRuntimeOverride existing, DomainSettingsPatch patch, int stamp) =>
        new()
        {
            ...
            DataCacheRespectAuthBypass = patch.DataCacheRespectAuthBypass ?? existing.DataCacheRespectAuthBypass,
            ...
            ClientTtl = patch.ClientTtl ?? existing.ClientTtl,
            ClientTtlMin = patch.ClientTtlMin ?? existing.ClientTtlMin,
            ...
            OutputCacheTtl = patch.OutputCacheTtl ?? existing.OutputCacheTtl,
            ...
            DataCacheRespectNoStore = patch.DataCacheRespectNoStore ?? existing.DataCacheRespectNoStore,
            ...
            DataCacheVaryOnPublicAddress = patch.DataCacheVaryOnPublicAddress ?? existing.DataCacheVaryOnPublicAddress,
            DataCacheVaryOnEncoding = patch.DataCacheVaryOnEncoding ?? existing.DataCacheVaryOnEncoding,
    ...
    private static DomainRuntimeOverride WithVersion(DomainRuntimeOverride existing, string version, int stamp) =>
        new()
        {
            ...
            DataCacheRespectAuthBypass = existing.DataCacheRespectAuthBypass,
            ...
            ClientTtl = existing.ClientTtl,
            ClientTtlMin = existing.ClientTtlMin,
            ScheduledUpdateUtc = existing.ScheduledUpdateUtc,
            ClientMustRevalidateNearUpdate = existing.ClientMustRevalidateNearUpdate,
            OutputCacheTtl = existing.OutputCacheTtl,
            DataCacheTtl = existing.DataCacheTtl,
            HardTtl = existing.HardTtl,
            FailSafe = existing.FailSafe,
            EagerRefreshRatio = existing.EagerRefreshRatio,
            Jitter = existing.Jitter,
            FactorySoftTimeout = existing.FactorySoftTimeout,
            FactoryHardTimeout = existing.FactoryHardTimeout,
            MaxItemBytes = existing.MaxItemBytes,
            DataCacheRespectNoStore = existing.DataCacheRespectNoStore,
            AllowBackgroundDistributed = existing.AllowBackgroundDistributed,
            AllowBackgroundBackplane = existing.AllowBackgroundBackplane,
            DataCacheVaryOnPublicAddress = existing.DataCacheVaryOnPublicAddress,
            DataCacheVaryOnEncoding = existing.DataCacheVaryOnEncoding,
