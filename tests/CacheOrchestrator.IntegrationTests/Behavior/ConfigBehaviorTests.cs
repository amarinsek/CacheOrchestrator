...
public class ConfigBehaviorTests
{
    ...
    public async Task Version_Change_OnRunningHost_ForcesOutputCacheMiss()
    {
        ...
        try
        {
            ...
            GetCacheOrchestratorHeader(r1).Should().Contain("oc=miss");
            GetCacheOrchestratorHeader(r1).Should().Contain("version=v1");
            ...
            GetCacheOrchestratorHeader(r2).Should().Contain("oc=hit");
            GetCacheOrchestratorHeader(r2).Should().Contain("version=v1");
            ...
            GetCacheOrchestratorHeader(r3).Should().Contain("oc=miss",
                "Version bump must change OC key so the v1 entry is not served");
            GetCacheOrchestratorHeader(r3).Should().Contain("version=v2");
            ...
            GetCacheOrchestratorHeader(r4).Should().Contain("oc=hit");
            GetCacheOrchestratorHeader(r4).Should().Contain("version=v2");
    ...
    private static string GetCacheOrchestratorHeader(HttpResponseMessage response) =>
        response.Headers.TryGetValues("X-CacheOrchestrator", out IEnumerable<string>? values)
            ? string.Join(",", values)
            : string.Empty;
