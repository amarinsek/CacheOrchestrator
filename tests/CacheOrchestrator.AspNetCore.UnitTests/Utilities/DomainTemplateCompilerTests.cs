...
public class DomainTemplateCompilerTests
{
    ...
    {
        Func<Func<HttpContext, string>> act = () => DomainTemplateCompiler.GetOrAdd(template!);
    ...
    {
        Func<Func<HttpContext, string>> act = () => DomainTemplateCompiler.GetOrAdd("catalog-{host");
    ...
    {
        Func<Func<HttpContext, string>> act = () => DomainTemplateCompiler.GetOrAdd("catalog-{route:}");
        act.Should().Throw<FormatException>();
    }
    ...
    {
        Func<HttpContext, string> resolver = DomainTemplateCompiler.GetOrAdd("product-catalog");
    ...
    {
        Func<HttpContext, string> resolver = DomainTemplateCompiler.GetOrAdd("tenant-{host}");
        var http = new DefaultHttpContext();
    ...
    {
        Func<HttpContext, string> resolver = DomainTemplateCompiler.GetOrAdd("tenant-{route:tenantId}");
        var http = new DefaultHttpContext();
    ...
    {
        Func<HttpContext, string> resolver = DomainTemplateCompiler.GetOrAdd("api-{header:X-Tenant}");
        var http = new DefaultHttpContext();
    ...
    {
        Func<HttpContext, string> resolver = DomainTemplateCompiler.GetOrAdd("filter-{query:sort}");
        var http = new DefaultHttpContext();
    ...
    public void GetOrAdd_CustomToken_UsesProvider()
    {
        ...
        Func<HttpContext, string> resolver = DomainTemplateCompiler.GetOrAdd("data-{custom:region}", custom);
        var http = new DefaultHttpContext();
    ...
    {
        Func<HttpContext, string> resolver = DomainTemplateCompiler.GetOrAdd("data2-{custom:missingkey}");
        var http = new DefaultHttpContext();
    ...
    {
        Func<HttpContext, string> resolver = DomainTemplateCompiler.GetOrAdd("{host}-tenant-{route:id}-v1");
        var http = new DefaultHttpContext();
    ...
    {
        Func<HttpContext, string> r1 = DomainTemplateCompiler.GetOrAdd("static-{host}");
        Func<HttpContext, string> r2 = DomainTemplateCompiler.GetOrAdd("static-{host}");
    ...
    {
        Func<HttpContext, string> r1 = DomainTemplateCompiler.GetOrAdd("a-{host}");
        Func<HttpContext, string> r2 = DomainTemplateCompiler.GetOrAdd("b-{host}");
    ...
    public void GetOrAdd_CustomProviders_DoNotPoisonSharedTemplateCache()
    {
        ...
        Func<HttpContext, string> withA = DomainTemplateCompiler.GetOrAdd(template, customA);
        withA(http).Should().Be("poison-check-eu-v1");

        // Later compile without providers must not reuse customA.
        Func<HttpContext, string> without = DomainTemplateCompiler.GetOrAdd(template);
        without(http).Should().Be("poison-check--v1");

        // Different provider maps must each bind their own value.
        Func<HttpContext, string> withB = DomainTemplateCompiler.GetOrAdd(template, customB);
    ...
    public void GetOrAdd_CustomProviders_AfterCachedPlain_StillUsesProviders()
    {
        ...
        Func<HttpContext, string> plain = DomainTemplateCompiler.GetOrAdd(template);
        ...
        Func<HttpContext, string> withCustom = DomainTemplateCompiler.GetOrAdd(template, custom);
        withCustom(http).Should().Be("plain-first-zone-a-v1");

        // Plain entry remains shared and unchanged.
        Func<HttpContext, string> plainAgain = DomainTemplateCompiler.GetOrAdd(template);
    ...
    public void GetOrAdd_EmptyCustomProviders_UsesSharedCache()
    {
        ...
        Func<HttpContext, string> r1 = DomainTemplateCompiler.GetOrAdd(template, empty);
        Func<HttpContext, string> r2 = DomainTemplateCompiler.GetOrAdd(template);
