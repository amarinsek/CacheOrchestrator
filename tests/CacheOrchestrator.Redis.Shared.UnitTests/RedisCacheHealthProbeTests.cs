...
public class RedisCacheHealthProbeTests
{
    ...
    {
        IConnectionMultiplexer mux = Substitute.For<IConnectionMultiplexer>();
        Func<RedisCacheHealthProbe> act = () => new RedisCacheHealthProbe(name!, mux);
    ...
    {
        Func<RedisCacheHealthProbe> act = () => new RedisCacheHealthProbe("redis:default", null!);
    ...
    public async Task ProbeAsync_WhenNotConnected_Throws()
    {
        ...
        Func<Task> act = () => probe.ProbeAsync(TestContext.Current.CancellationToken);
