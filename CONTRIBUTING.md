...
All eleven NuGet packages (meta, Core, AspNetCore, FusionCache, HybridCache, Redis.Shared, AspNetCore.Redis, FusionCache.Redis, Redis meta, HttpBus, EFCore.Invalidation) share the same MinVer version and `PACKAGE_RELEASE_NOTES.md`. `Redis.Shared` is support/transitive (not an app install target). Admin Console is Docker/GHCR only (`IsPackable=false`).
